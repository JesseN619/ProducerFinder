...
export const Playlist = () => {
  ...
  const removeFromPlaylist = useCallback(
    async (e: MouseEvent, songId: string) => {
    ...
    const displayPlaylist = async () => {
      ...
      if (arrOfSongs.length !== 0) {
        ...
        for (let i = 0; i < arrOfSongs.length; i++) {
          ...
          delBtn.className =
            "bg-red-600 hover:bg-red-700 px-2 rounded text-xl text-gray-200 font-semibold ml-auto";
          delBtn.innerHTML = "x";
          delBtn.addEventListener("click", (e) =>
            removeFromPlaylist(e, trackId)
          );
  ...
  }, [playlistId, headers, removeFromPlaylist, ul]);
  ...
  useEffect(() => {
    const addToPlaylist = async () => {
      ...
      const result = await fetch(
        `https://api.spotify.com/v1/playlists/${playlistId}/tracks?uris=spotify:track:${songId}`,
        {
          method: "POST",
          headers: headers,
        }
  ...
  }, [songId, headers, playlistId, removeFromPlaylist, ul]);

  return (
    <div className="w-6/12 mx-auto">
      <h2 id="playlist-name" className="text-3xl text-center mt-2 mb-5 rounded">
        {useSelector(selectPlaylistName)}
      </h2>
